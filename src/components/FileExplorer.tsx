...
const FileItem = ({ node, depth, onSelect, selectedFile }: { node: FileNode; depth: number; onSelect: (f: FileNode) => void; selectedFile: string | null }) => {
    ...
    return (
      <div>
        ...
        {isOpen && node.children?.map((child, i) => (
          <FileItem key={i} node={child} depth={depth + 1} onSelect={onSelect} selectedFile={selectedFile} />
        ))}
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 w-full px-2 py-1 text-sm rounded transition-colors ${
        selectedFile === node.name ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-secondary/50"
      }`}
      style={{ paddingLeft: `${depth * 12 + 8}px` }}
    >
...
export const FileExplorer = ({ onSelectFile, selectedFile }: FileExplorerProps) => {
  return (
    <div className="flex flex-col h-full bg-card border-r border-border">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {mockFiles.map((node, i) => (
          <FileItem key={i} node={node} depth={0} onSelect={onSelectFile} selectedFile={selectedFile} />
        ))}
      </div>
    </div>
  );
